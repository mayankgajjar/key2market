  ...
  <table class="table table-bordered" id="user-table" style="text-align:center;">
    ...
    <tbody>
        ...
        <tr style="background-color:  white;">
            ...
            <td>{{ encryption_decryption_data($tdata->connection_name,'d') }}</td>
            <td>{{ $tdata->schema_name }}</td>
            <td>{{ $tdata->table_name }}</td>
            <td>{{ $tdata->date_column_name }}</td>
            <td>{{ $tdata->id_column_name }}</td>
            <td>
                <a href="{{ route('monitor.edit',$tdata->id) }}"><i class="fa fa-pencil" title = "Edit Table Data"></i></a> &nbsp;
                <a><span class="del_table_data" data-custom-value="{{$tdata->id}}"><i class="fa fa-trash" title = "Delete Table Data" style="cursor:  pointer;"></i></span></a> &nbsp;
            </td>
        </tr>
        @endforeach
    </tbody>
...
<script>
    $(document).on("click", ".del_table_data", function () {
        ...
        swal({
            title: "Are you sure?",
            text: "You will not be able to recover this Monitor Table!",
            type: "warning",
          ...
          function(isConfirm) {
            if (isConfirm) {
              ...
              swal("Cancelled", "Monitor Table is safe", "error");
            }
        });
    })
</script>
